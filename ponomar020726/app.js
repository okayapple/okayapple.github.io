...
function renderLayerStyleControls(layer, panel) {
  ...
  if (layer.kind === "Point") {
    container.innerHTML = '<div class="centroid-controls"><div class="category-heading"><span>Point Labels</span></div><div class="centroid-grid"><label class="style-control"><span>Show labels</span><select class="point-label-visible"><option value="1">Yes</option><option value="0">No</option></select></label><label class="style-control"><span>Min zoom</span><select class="point-label-minzoom"><option value="10">10</option><option value="12">12</option><option value="14">14</option><option value="16">16</option><option value="18">18</option></select></label></div></div><div class="centroid-controls author-only"><div class="category-heading"><span>Point Colors</span></div><div class="centroid-grid"><label class="style-control"><span>Marker</span><input class="base-fill-color" type="color" aria-label="Point marker color"></label><label class="style-control"><span>Outline</span><input class="base-line-color" type="color" aria-label="Point outline color"></label></div></div>';
  ...
  container.innerHTML = '<label class="style-control"><span>View</span><select class="style-mode"><option value="both">Fill and outline</option><option value="fill">Fill only</option><option value="line">Outline only</option></select></label><label class="style-control"><span>Layer opacity</span><select class="style-opacity-input"><option value="0.25">25%</option><option value="0.5">50%</option><option value="0.75">75%</option><option value="1">100%</option></select></label><label class="style-control"><span>Outline width</span><select class="style-width-input"><option value="1">1 px</option><option value="2">2 px</option><option value="3">3 px</option><option value="4">4 px</option><option value="5">5 px</option></select></label><div class="centroid-controls author-only"><div class="category-heading"><span>Base colors</span></div><div class="centroid-grid"><label class="style-control"><span>Fill</span><input class="base-fill-color" type="color" aria-label="Base fill color"></label><label class="style-control"><span>Outline</span><input class="base-line-color" type="color" aria-label="Base outline color"></label></div></div><div class="centroid-controls author-only"><div class="category-heading"><span>Centroids</span></div><div class="centroid-grid"><label class="style-control"><span>Mode</span><select class="centroid-mode"><option value="none">Hide</option><option value="marker">Marker</option><option value="label">Label</option><option value="both">Marker + label</option></select></label><label class="style-control"><span>Label field</span><select class="centroid-field"><option value="">No field</option>' + fields + '</select></label></div></div>';
...
async function applyPersistedStyleOverrides() {
  try {
    ...
    console.warn("style.json was not loaded", error);
  }
}
...
async function persistStyleOverrides() {
  ...
  if (!response.ok) throw new Error("style save failed: HTTP " + response.status);
  try {
