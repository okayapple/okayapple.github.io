...
function createMap() {
  ...
  state.map.on("zoom", () => { updateMapStatus(); refreshCentroidLayers(); updateLabelVisibilityUi(); });
...
function renderExchangePanel() {
  ...
  if (labelsToggleText) labelsToggleText.textContent = "\u041f\u043e\u0434\u043f\u0438\u0441\u0438 \u043e\u0431\u043c\u0435\u043d\u043e\u0432";
...
function exchangeLegendGroup(layer, group, query = "") {
  ...
  return '<details class="exchange-group" open><summary class="exchange-summary"><label class="exchange-group-toggle"><input type="checkbox" data-exchange-group-toggle="' + groupToken + '"' + checked + '><span class="exchange-category-swatch"><span class="exchange-swatch line" style="color:' + groupColor + '"></span></span><span class="exchange-category-label"><span>' + escapeHtml(exchangeValueFallback(group.value)) + '</span></span></label><button type="button" class="exchange-fit-button" data-exchange-fit="' + groupToken + '" title="Fit group">fit</button></summary><div class="exchange-group-body">' + subgroupRows + '</div></details>';
}
...
function addExchangeLayer(layer) {
  ...
  state.map.addLayer({
    id:`${layer.id}-exchange-border-halo`,
    type:'line',
    source:layer.id,
    layout:{visibility:'none'},
    paint:{
...
function applyExchangeVisibility(layer) {
  ...
  if (state.map.getLayer(`${layer.id}-exchange-border-halo`)) {
    state.map.setLayoutProperty(`${layer.id}-exchange-border-halo`, 'visibility', visibility);
    state.map.setFilter(`${layer.id}-exchange-border-halo`, filter);
  }
...
function isConfiguredLayerVisible(layer) {
  if (layer.exchange) return ["exchange-fill", "exchange-border-halo", "exchange-border", "exchange-labels"].some((part) => state.map?.getLayer(layer.id + "-" + part) && state.map.getLayoutProperty(layer.id + "-" + part, "visibility") !== "none");
