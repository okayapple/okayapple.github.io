...
function createMap() {
  ...
  state.map.on("zoom", updateMapStatus);
...
function initializeExchangeState() {
  ...
  const borderValues = uniqueExchangeValues(layer, layer.exchange?.borderField);
  const fillValues = uniqueExchangeValues(layer, layer.exchange?.patternField);
  const preset = state.exchangePreset || {};
  state.exchange = {
    visible: preset.visible !== false && (state.layerVisible.get(layer.id) ?? (layer.visible !== false)),
    borderValues: exchangeEnabledMap(borderValues, preset.borderValues),
  };
  layer.exchange._borderColors = buildExchangeColorMap(layer, layer.exchange.borderField, 18, 54);
...
function setAllExchangeCategories(enabled) {
  ...
  Object.keys(state.exchange.borderValues || {}).forEach((value) => {
    state.exchange.borderValues[value] = Boolean(enabled);
  });
  applyExchangeVisibility(layer);
  renderExchangePanel();
}

function setExchangeCategoryToggle(value, enabled) {
  const layer = exchangeLayer();
  if (!layer || !state.exchange) return;
  state.exchange.borderValues[value] = Boolean(enabled);
  applyExchangeVisibility(layer);
...
function exchangeAllCategoriesHidden() {
  const values = Object.values(state.exchange?.borderValues || {});
  return values.length ? values.every((value) => value === false) : false;
}
...
function renderExchangePanel() {
  ...
  const listScroll = legend?.querySelector('.exchange-group-body')?.scrollTop || 0;
  ...
  if (title) title.textContent = '\u041e\u0431\u043c\u0435\u043d\u044b';
  const hideAllText = document.querySelector('#exchangeHideAllToggle + span');
  if (hideAllText) hideAllText.textContent = '\u0421\u043a\u0440\u044b\u0442\u044c \u0432\u0441\u0435';
  if (legend) legend.innerHTML = exchangeLegendHtml(layer);
  bindExchangeLegendControls();
  if (body) body.scrollTop = bodyScroll;
  const nextList = legend?.querySelector('.exchange-group-body');
  if (nextList) nextList.scrollTop = listScroll;
}

function exchangeLegendHtml(layer) {
  const parts = [];
  const borderRows = exchangeCategoryRowsHtml(layer.exchange?._borderColors || {}, state.exchange?.borderValues || {});
  if (borderRows) parts.push(exchangeLegendGroup(exchangeFieldLabel(layer, layer.exchange?.borderField), borderRows));
  return parts.join('');
}

function exchangeLegendGroup(title, rows) {
  return '<details class="exchange-group" open><summary class="exchange-summary">' + escapeHtml(title) + '</summary><div class="exchange-group-body">' + rows + '</div></details>';
}

function exchangeCategoryRowsHtml(valueMap, enabledMap) {
  return Object.entries(valueMap).map(([value, color]) => {
    const checked = enabledMap[value] !== false ? ' checked' : '';
    const token = exchangeValueToken(value);
    const label = exchangeValueFallback(value);
    return '<label class="exchange-category-item"><input type="checkbox" data-exchange-value="' + token + '"' + checked + '><span class="exchange-category-swatch"><span class="exchange-swatch line" style="color:' + color + '"></span></span><span class="exchange-category-label">' + escapeHtml(label) + '</span><button type="button" class="exchange-fit-button" data-exchange-fit="' + token + '" title="Fit category">fit</button></label>';
  }).join('');
}

function bindExchangeLegendControls() {
  document.querySelectorAll('[data-exchange-value]').forEach((input) => {
    input.addEventListener('change', (event) => {
      const target = event.currentTarget;
      const value = decodeExchangeValueToken(target.dataset.exchangeValue || '');
      setExchangeCategoryToggle(value, target.checked);
    });
  });
...
function exchangeFilterExpression(fieldName, enabledMap) {
  if (!fieldName || !enabledMap) return null;
  const enabledValues = Object.entries(enabledMap).filter(([, enabled]) => enabled !== false).map(([value]) => value);
  if (enabledValues.length === Object.keys(enabledMap).length) return null;
  if (!enabledValues.length) return ['==', ['literal', true], false];
  return ['in', ['coalesce', ['to-string', ['get', fieldName]], ''], ['literal', enabledValues]];
}
...
function applyExchangeVisibility(layer) {
  ...
  const hasVisibleCategories = Object.values(state.exchange.borderValues || {}).some((value) => value !== false);
  const visibility = master && hasVisibleCategories ? 'visible' : 'none';
  const filter = exchangeFilterExpression(layer.exchange?.borderField, state.exchange.borderValues);
  ...
  if (state.map.getLayer(`${layer.id}-exchange-labels`)) {
    state.map.setLayoutProperty(`${layer.id}-exchange-labels`, 'visibility', visibility);
  }
  renderExchangePanel();
}
...
function createExchangeLabelCollection(layer) {
  ...
  const enabledValues = state.exchange?.borderValues || {};
  const features = (state.datasets.get(layer.id)?.features || []).map((feature, index) => {
    ...
    if (enabledValues[borderValue] === false) return null;
    ...
    return {
      ...
      properties:{ exchange_label: `${exchangeValueFallback(borderValue)}\n${exchangeValueFallback(fillValue)}` },
...
function applyCombinedLayerFilter(layer) {
  ...
  ["fill", "line"].forEach((part) => { if (state.map.getLayer(layer.id + "-" + part)) state.map.setFilter(layer.id + "-" + part, expression); });
...
function renderLayerStyleControls(layer, panel) {
  ...
  if (!container || layer.kind !== "Polygon") return;
  ...
  container.innerHTML = '<label class="style-control"><span>Вид</span><select class="style-mode"><option value="both">Заливка и контур</option><option value="fill">Только заливка</option><option value="line">Только контур</option></select></label><label class="style-control"><span>Прозрачность слоя</span><select class="style-opacity-input"><option value="0.25">25%</option><option value="0.5">50%</option><option value="0.75">75%</option><option value="1">100%</option></select></label><label class="style-control"><span>Толщина контура</span><select class="style-width-input"><option value="1">1 px</option><option value="2">2 px</option><option value="3">3 px</option><option value="4">4 px</option><option value="5">5 px</option></select></label><div class="centroid-controls author-only"><div class="category-heading"><span>Базовые цвета</span></div><div class="centroid-grid"><label class="style-control"><span>Заливка</span><input class="base-fill-color" type="color" aria-label="Базовый цвет заливки"></label><label class="style-control"><span>Контур</span><input class="base-line-color" type="color" aria-label="Базовый цвет контура"></label></div></div><div class="centroid-controls author-only"><div class="category-heading"><span>Центроиды</span></div><div class="centroid-grid"><label class="style-control"><span>Режим</span><select class="centroid-mode"><option value="none">Не показывать</option><option value="marker">Маркер</option><option value="label">Подпись</option><option value="both">Маркер + подпись</option></select></label><label class="style-control"><span>Поле подписи</span><select class="centroid-field"><option value="">Без поля</option>' + fields + '</select></label></div></div>';
...
function layerRenderOptions(layer) {
  if (!state.layerRenderOptions.has(layer.id)) state.layerRenderOptions.set(layer.id, {mode:"both", opacity:nearestOpacityStep(layer.style?.fillOpacity ?? 0.5), width:nearestWidthStep(layer.style?.lineWidth ?? 1)});
...
function isConfiguredLayerVisible(layer) {
  ...
  return ["fill","line","other-line"].some((part) => state.map?.getLayer(layer.id + "-" + part) && state.map.getLayoutProperty(layer.id + "-" + part, "visibility") !== "none");
}
...
function handleMapClick(event) {
  ...
  if (!layer) return;
  const layerIds = [layer.id + "-fill", layer.id + "-line", layer.id + "-other-line"].filter((id) => state.map.getLayer(id));
...
function bindUi() {
  ...
  document.getElementById("searchClear").addEventListener("click", () => { document.getElementById("searchInput").value=""; document.getElementById("searchInput").focus(); });
...
function renderSearchFields() {
  ...
  input.placeholder = selectedField ? "Поиск по атрибуту поиска" : "Для поиска задайте атрибут поиска в QGIS";
}
...
function searchFeatures(event) {
  ...
  const query = document.getElementById("searchInput").value.trim().toLocaleLowerCase("ru");
  if (!query) return;
  const layer = interactiveLayer();
  if (!layer) return showToast("\u041d\u0435\u0442 \u0438\u043d\u0442\u0435\u0440\u0430\u043a\u0442\u0438\u0432\u043d\u043e\u0433\u043e \u0441\u043b\u043e\u044f \u0434\u043b\u044f \u043f\u043e\u0438\u0441\u043a\u0430");
  const selectedField = layer.defaultSearchField || layer.searchFields?.[0] || "";
  if (!selectedField) return showToast("Для интерактивного слоя не настроено поле поиска");
  const features = state.datasets.get(layer.id)?.features || [];
  const feature = features.find((item) => String(item.properties?.[selectedField] ?? "").toLocaleLowerCase("ru").includes(query));
  if (feature) { selectFeature(layer,feature,{zoom:true}); showToast("\u041d\u0430\u0439\u0434\u0435\u043d\u043e: " + layer.label); return; }
  showToast("\u041e\u0431\u044a\u0435\u043a\u0442 \u043d\u0435 \u043d\u0430\u0439\u0434\u0435\u043d \u0432 \u0438\u043d\u0442\u0435\u0440\u0430\u043a\u0442\u0438\u0432\u043d\u043e\u043c \u0441\u043b\u043e\u0435");
}
...
function snapshotOriginalLayerStyles() {
  state.originalLayerStyles = new Map(state.config.layers.map((layer) => {
    ...
    return [layer.id, cloneValue({
      ...
      renderOptions:{mode:"both", opacity:nearestOpacityStep(layer.style?.fillOpacity ?? 0.5), width:nearestWidthStep(layer.style?.lineWidth ?? 1)},
      enabledCategories
    })];
  }));
}
...
function collectStyleOverrides() {
  ...
  state.config.layers.forEach((layer) => {
    ...
    if (currentOptions.mode !== original.renderOptions?.mode || currentOptions.opacity !== original.renderOptions?.opacity || currentOptions.width !== original.renderOptions?.width) override.renderOptions = currentOptions;
  ...
  if (state.exchange) payload.exchange = {visible:state.exchange.visible, borderValues:state.exchange.borderValues};
  return payload;
}
...
async function copyStyleLink() {
  ...
  try {
    ...
    showToast("Ссылка со стилем скопирована");
  } catch (error) {
    showToast("Ссылка подготовлена в поле ниже");
  }
}
